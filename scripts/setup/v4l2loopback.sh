...
EOF
  sudo install -m 0644 "$TMPDIR_PERSIST/modprobe.conf" "$CONF_MODPROBE"

  cat > "$TMPDIR_PERSIST/modules-load.conf" <<EOF
# Auto-generated by StudioCast: load v4l2loopback on boot
v4l2loopback
EOF
  sudo install -m 0644 "$TMPDIR_PERSIST/modules-load.conf" "$CONF_MODULES"

  rm -rf "$TMPDIR_PERSIST"
